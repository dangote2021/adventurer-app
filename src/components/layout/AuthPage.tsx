...
export default function AuthPage() {
  ...
  // Note: signInWithApple existe dans le provider — on le réactivera quand on sortira la version iOS native.
  // Pour la v1 Android-only + PWA iOS, on se limite à Google + email pour ne pas afficher un bouton non configuré.
  ...
  const handleGoogleLogin = async () => {
    ...
    if (error) {
      ...
      setError(t('auth.googleError', language));
  ...
  return (
    <div className="min-h-screen bg-[var(--bg)] flex flex-col max-w-[430px] mx-auto">
      ...
      <div className="flex-1 px-6 py-4 space-y-5">
        {/* Mode Toggle */}
        <div className="flex bg-white/5 rounded-xl p-1" role="tablist" aria-label={t('auth.modeTablist', language)}>
          <button
            type="button"
            role="tab"
        ...
        <button
          type="button"
          className="w-full py-3 bg-white text-gray-900 rounded-xl font-semibold flex items-center justify-center gap-3 hover:bg-gray-100 transition focus-visible:outline focus-visible:outline-2 focus-visible:outline-[var(--accent)] disabled:opacity-60 disabled:cursor-wait"
          onClick={handleGoogleLogin}
          disabled={googleLoading}
        >
          {googleLoading ? (
            <svg className="w-5 h-5 animate-spin" viewBox="0 0 24 24" fill="none" aria-hidden="true">
              <circle cx="12" cy="12" r="10" stroke="#4285F4" strokeWidth="3" strokeDasharray="50 20" strokeLinecap="round" />
            </svg>
          ) : (
          ...
          {googleLoading ? t('auth.connectingGoogle', language) : t('auth.continueGoogle', language)}
        </button>
        ...
        <form onSubmit={handleEmailSubmit} className="space-y-3" noValidate>
          ...
          {mode === 'signup' && (
            <div className="flex items-start gap-2 py-2">
              <input
                ...
                aria-required="true"
                aria-label={t('auth.ariaAcceptTerms', language)}
              />
          ...
          <button
            ...
            aria-label={mode === 'login' ? t('auth.ariaSubmitLogin', language) : t('auth.ariaSubmitSignup', language)}
          >

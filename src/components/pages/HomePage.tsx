...
export default function HomePage() {
  ...
  return (
    <main className="min-h-screen bg-[var(--bg)] text-white pb-28 max-w-[430px] mx-auto">
      ...
      {/* SOCIAL ACTIVITY FEED — empty state honnête (panel V5 / Sara).
          Le bloc fakeFeed (12 profils Léa M./Thomas K./Sarah M. 8a+...) faisait
          passer l'app pour un faux univers communautaire dès qu'un user
          loggait sa première activité. Anti-pattern identique à Mini-Ligue
          qu'on a fixée. Tant qu'on n'a pas un vrai feed Supabase basé sur les
          amis ajoutés, on affiche UNIQUEMENT l'empty state + CTA d'invitation,
          peu importe l'état de activityLog. Le fakeFeed code est gardé en
          dead-code commenté plus bas pour référence quand on rebranchera un
          vrai feed amis. */}
      <section className="px-4 sm:px-6 py-6">
        ...
        {/* /false dead-code block — fakeFeed conservé pour référence,
            à remplacer par un vrai feed amis Supabase quand on aura branché
            le système d'amis. */}
      </section>
